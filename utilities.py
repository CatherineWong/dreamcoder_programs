...
def limit_virtual_memory_fn(max_mem_per_enumeration_thread):
    ...
    def limit_virtual_memory_to_max():
        resource.setrlimit(
            resource.RLIMIT_AS, (max_mem_per_enumeration_thread, resource.RLIM_INFINITY)
        )
...
def convert_iterations_to_training_task_epochs(args, train_tasks):
    ...
    if use_epochs and args["taskBatchSize"] is not None:
        ...
        print(
            f"Now running for n={intended_epochs} epochs, which with {len(train_tasks)} training tasks is n={args['iterations']} iterations."
        )
...
def tuplify(x):
    ...
    if isinstance(x, dict):
        return frozendict({tuplify(k): tuplify(v) for (k, v) in x.items()})
    return x


def unfrozendict(x):
    if isinstance(x, (list, tuple)):
        return tuple(unfrozendict(z) for z in x)
    if isinstance(x, frozendict):
        return {unfrozendict(k): unfrozendict(v) for (k, v) in x.items()}
    return x
